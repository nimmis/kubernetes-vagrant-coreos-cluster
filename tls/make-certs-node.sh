...
openssl req -new -key $pem_node_key -out $pem_node_csr -subj "/CN=system:node:__NAME__/O=system:masters" -config /tmp/openssl.cnf
# openssl req -new -key $pem_node_key -out $pem_node_csr -subj "/CN=system:node:__NAME__/O=system:nodes" -config /tmp/openssl.cnf
